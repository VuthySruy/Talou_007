...
public class ReadyUIHandler : NetworkBehaviour
{
    ...
    public void OnChangeCharcterHelmet()
    {
        ...
        NetworkPlayer.Local.GetComponent<CharacterOutfitHandler>().OnCycleHelmet();
    }
    ...
    public void OnChangeCharcterUpperBody()
    {
        if (isReady)
            return;

        NetworkPlayer.Local.GetComponent<CharacterOutfitHandler>().OnCycleUpperBody();
    }

    public void OnChangeCharcterLowerBody()
    {
        if (isReady)
            return;

        NetworkPlayer.Local.GetComponent<CharacterOutfitHandler>().OnCycleLowerBody();
    }

    public void OnChangeCharcterFeet()
    {
        if (isReady)
            return;

        NetworkPlayer.Local.GetComponent<CharacterOutfitHandler>().OnCycleFeet();
